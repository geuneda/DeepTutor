...
if (!fs.existsSync(enRoot)) {
  console.error(`[i18n:parity] Missing locales root: ${enRoot}`);
  process.exit(2);
}

// Every sibling of locales/en is a translation of it, so they all get checked
// against the English source rather than only the one locale we started with.
const targets = fs
...
if (!targets.length) {
  console.error("[i18n:parity] No translation locales found next to locales/en");
  process.exit(2);
}
...
console.log(`[i18n:parity] OK (${targets.join(", ")})`);
